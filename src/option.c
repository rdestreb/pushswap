...
/*   Updated: 2015/02/20 17:05:53 by rdestreb         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	ft_putstr_fd("usage: ./push_swap -help [-vnbpi] [digit1 digit2 ...]\n", 2);
	...
	ft_putstr_fd("    -p  use only the 'Push-Swap' algo\n", 2);
...
static void	illegal_option(char *arg)
{
	...
	while (arg[++i])
	{
		if (arg[i] != 'v' && arg[i] != 'n' && arg[i] != 'b' && arg[i] != 'p'
			&& arg[i] != 'i')
		{
			ft_putstr_fd("push_swap: illegal option -- ", 2);
			ft_putchar_fd(arg[i], 2);
			ft_putstr_fd("\nusage: push_swap [-vnbpi] [digit1 digit2 ...]\n", 2);
			exit(2);
		}
	}
}
